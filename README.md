...
- activation sends run as an in-memory background job and the UI shows progress, avoiding reverse-proxy timeouts during a batch
...
- If the app container is restarted during an activation batch, the in-progress background job stops. Users already sent remain protected by the cooldown record.
